            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    if grad.dim() > 1:
                        ...
                        # grad.size(1) is only C_in, but we need C_in * H * W for R
                ...
                if grad.dim() > 1:
                    ...
                    # Use the same flattened view as Init
                    ...
                    inv_L = self._matrix_power(state['precond_L'] / state['step'] + eps * torch.eye(grad_mat.size(0), device=p.device), -1/4)

                    inv_R = self._matrix_power(state['precond_R'] / state['step'] + eps * torch.eye(grad_mat.size(1), device=p.device), -1/4)
                    ...
                    if grad.dim() == 2:
                        ...
                        # Reconstruct Conv gradient from flattened preconditioned matrix

                        # (C_out, Flat) -> (C_out, C_in, H, W)
